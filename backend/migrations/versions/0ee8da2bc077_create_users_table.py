...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('associated_restaurant_id', sa.Integer(), nullable=True),  # FK نضيفه لاحقًا
    ...
    # بعد users، نخلق جدول restaurants بدون FK إلى users
    op.create_table(
        'restaurants',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('name', sa.String(length=100), nullable=False),
        ...
        sa.Column('manager_id', sa.Integer(), nullable=False),  # FK نضيفه لاحقًا
    ...
    op.drop_table('spatial_ref_sys')
        ...
        # أخيرًا نضيف FK بعد إنشاء كلا الجدولين لتجنب الحلقة
